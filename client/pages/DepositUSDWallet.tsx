...
export default function DepositUSDWallet() {
  ...
  const handlePaymentMethod = (method: string) => {
    navigate('/payment-gateway', { state: { amount, currency: 'USD', method } });
  };
    ...
    <div className="flex min-h-screen flex-col bg-slate-900 px-4 py-6">
      ...
      <div className="flex-1 max-w-sm w-full mx-auto space-y-4">
        <div className="space-y-2">
          <h1 className="text-2xl font-bold text-white">Deposit USD Wallet</h1>
          <p className="text-sm text-slate-400">Add USD funds to your wallet</p>
        </div>
          ...
          <>
            <Card className="border-0 bg-slate-800 p-4 space-y-4">
              <div className="bg-slate-700 p-3 rounded-lg">
                <p className="text-xs text-slate-400">Amount to deposit</p>
